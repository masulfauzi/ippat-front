  ...
  <div class="bg-surface min-h-screen">
    ...
    <main class="lg:ml-64 min-h-screen">
      ...
      <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Informasi detail wilayah</p>
      </div>
      ...
      <div v-if="isLoading" class="flex justify-center items-center py-12">
        <div class="text-center">
          ...
          <p class="mt-3 text-slate-600">Memuat detail wilayah...</p>
        </div>
      </div>
      ...
      <div v-else-if="selectedJurusan" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow border border-slate-200 p-6">
          <h3 class="text-sm font-semibold text-slate-600 uppercase mb-2">Nama Wilayah</h3>
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="text-slate-600 mt-4">Wilayah tidak ditemukan</p>
        <button
          @click="handleBack"
...
onMounted(async () => {
  try {
    ...
    error.value = 'Gagal memuat detail wilayah'
  }
})
...
const handleDelete = async () => {
  if (await $confirm('Yakin ingin menghapus wilayah ini? Tindakan ini tidak dapat dibatalkan.', { title: 'Konfirmasi Hapus' })) {
    try {
      await jurusanStore.deleteJurusan(jurusanId)
      router.push({ name: 'jurusan.list' })
    } catch (err) {
      error.value = jurusanStore.error || 'Gagal menghapus wilayah'
    }
  }
}
